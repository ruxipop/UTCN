...

{
        ...
        pthread_mutex_lock(s->lockTurn);

        while(turn != s->id)

                pthread_cond_wait(s->condTurn, s->lockTurn);
pthread_mutex_unlock(s->lockTurn);
        ...
        while(balance < AMOUNT_WITHDRAW) {
                ...
                pthread_cond_broadcast(s->condTurn);
...

{
        ...
        pthread_cond_t condTurn = PTHREAD_COND_INITIALIZER;
        ...
        for(i=0; i<NR_THREADS; i++) {
                ...
                params[i].lockTurn = &lockTurn;

                params[i].condTurn = &condTurn;
